...
initialization_atrisk <- function(inputObj, num_pospop) {
  ...
  nodes_TEMP = inputObj$popprobs$nodes %>%
    group_by(rwhap) %>%
    mutate(rwhapstagemult = sum(rwhapstagemult)) %>%
    filter(stage == "hiv") %>%
    mutate(stage = "at_risk")

  rna_TEMP = inputObj$popprobs$rna %>%
    filter(stage == "hiv") %>%
    mutate(stage = "at_risk")

  combinedDemos <- left_join(inputObj$popprobs$atrisk, inputObj$popprobs$service,
                             by = c("agegroup", "gender", "race", "risk", "rwhap")) %>%
    left_join(nodes_TEMP, by = "rwhap", relationship = "many-to-many") %>%
    left_join(rna_TEMP, by = "stage", relationship = "many-to-many")
  ...
  hivData <- dataShell %>%
    left_join(combinedDemos, by = c("agegroup", "gender", "race", "risk",
                                    "stage", "rnaset", "rwhap")) %>%
    mutate(totpop = inputObj$pospopulationsize * inputObj$negpopulationmult,
           pop    = as.numeric(NA))
  ...
  hivData = hivData %>%
    mutate(pop = totpop*rwhapstagemult*rnamult*atrisk)
  ...
  if (inputObj$testflag) stopifnot(all.equal(sum(checkthis$pop),
                                             inputObj$pospopulationsize * inputObj$negpopulationmult,
                                             tolerance = 0.5))
  ...
  if (inputObj$testflag) stopifnot(all.equal(sum(hivData$newpop),
                                             inputObj$pospopulationsize * inputObj$negpopulationmul,
                                             tolerance = 0.5))
